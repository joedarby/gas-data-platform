...
def get_results(engine):
    ...
    query = "SELECT a.timestamp, a.location, a.value " \
                  ...
                  "     FROM %s " \
                  "     GROUP BY location " \
                  ") b ON a.location = b.location AND a.timestamp = b.timestamp" % ("NorwayData", "NorwayData")
...
def create_response(results):
    ...
    if results:
        ...
        dataset = {"norwayFlows": terminals}

        response = {"statusCode": 200, "headers": {"Content-Type": "application/json"}, "body": str(dataset)}
    else:
