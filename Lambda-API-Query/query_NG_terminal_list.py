...
def get_results(engine):
    ...
    query = "SELECT a.timestamp, a.location, a.value " \
                  ...
                  "     FROM %s " \
                  "     GROUP BY location " \
                  ") b ON a.location = b.location AND a.timestamp = b.timestamp" % ("NGData", "NGData")
...
def create_response(results):
    ...
    if results:
        ...
        for r in results:
            ...
            if location in ng_mapping.NG_terminal_map.keys():
                p = ng_mapping.Pipeline(location, value, timestamp)
                t = ng_mapping.NG_terminal_map[location]
